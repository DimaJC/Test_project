...
function FilterSystem () {
    ...
    const ItemRender = () => Users.map((item) => {
        const [year] = item.joinDate.split('-').map(Number)
        return (isActive !== undefined ? item.isActive === isActive : true) 
        && (gender !== undefined && gender !== 'All' ? item.gender === gender : true)
        && (country !== undefined && country !== 'All' ? item.country === country : true)
        && (AgeRange !== undefined && AgeRange !== 'All' ? getage(item.age) : true)
        && (JoinDate !== undefined && JoinDate !== 'All' ? Number(JoinDate)=== year : true)
        &&(
        ...
        <div className="main">
            <div className="Filter">
                ...
                <div className="Buttons">
                    <h1>ActiveUsers:</h1>
                    <label htmlFor="iSActive">Active</label> <input type="checkbox" name="iSActive" id="iSActive" onClick={HendleisActive} checked={isActive === true} />
                    <div className="genderinputs">
                        <h1>Genders:</h1>
                        <label htmlFor="All">All</label> <input type="radio" name="Gender" id="All" onChange={HendleGender} checked={gender === 'All'}/>
                        <br />
                        <label htmlFor="Male">Male</label> <input type="radio" name="Gender" id="Male" onChange={HendleGender} checked={gender === 'Male'} />
                        <br />
                        <label htmlFor="Female">Female</label> <input type="radio" name="Gender" id="Female" onChange={HendleGender} checked={gender === 'Female'} />
                    </div> 
            ...
            <div className="PepplesContainer">
                {ItemRender()}
            </div>
        </div>
    );
